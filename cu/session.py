...
class ShellSessionError(Exception):
    ...
    :func:`ShellSession.popen <cu.session.ShellSession.popen>`
    '''
    pass


class MarkedPipe(object):
    '''A pipe-like object from which you can read lines; the pipe will return
    report EOF (the empty string) when a special marker is detected.
    '''
    ...
    def readline(self):
        '''Reads the next line from the pipe; returns '' when the special marker
        is reached.  Raises ``EOFError`` if the underlying pipe has closed.
        '''
...
class SessionPopen(object):
    '''A shell-session-based ``Popen``-like object (has the following
    attributes: ``stdin``, ``stdout``, ``stderr``, ``returncode``).
    '''
...
class ShellSession(object):
    '''An abstraction layer over *shell sessions*. A shell session is the
    execution of an interactive shell (``/bin/sh`` or something compatible),
    over which you may run commands (sent over stdin). The output of is then
    read from stdout and stderr. Shell sessions are less 'robust' than executing
    a process on its own, and they are susseptible to all sorts of
    malformatted-strings attacks, and there is little benefit from using them
    locally.  However, they can greatly speed up remote connections, and are
    required for the implementation of :class:`SshMachine
    <cu.remote_machine.SshMachine>`, as they allow us to send multiple commands
    over a single SSH connection (setting up separate SSH connections incurs a
    high overhead). Try to avoid using shell sessions, unless you know what
    you're doing.
    ...
    def popen(self, cmd):
        ...
        :param cmd: The command (string or :class:`Command <cu.command.BaseCommand>` object)
                    to run
        :returns: A :class:`SessionPopen <cu.session.SessionPopen>` instance
        '''
    ...
    def run(self, cmd, retcode=0):
        ...
        :param cmd: The command (string or :class:`Command
                <cu.command.BaseCommand>` object) to run
        :param retcode: The expected return code (0 by default). Set to ``None``
                in order to ignore erroneous return codes
